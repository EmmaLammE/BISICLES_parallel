...
contains
  ...
  !!! still working on it
  subroutine Pf_Main(AmrIceHolderPtr,pf_comm_fromBisicles,crse_nsteps,dt_bisicles,Tfin_bisicles,maxStep_bisicles,numGridPointsBisicles,pf_num_procs_per_time,PF_VERBOSE) bind(c, name="Pf_Main")
    use pfasst             !< contains all neccessary mods for pfasst rountines. pfasst/src/pfasst.f90
    use pf_mod_dtype
    use pf_mod_mpi
    use pf_mod_comm_mpi
    ...
    do i = 1,num_repeat_pf_run
      ...
      elapse_time_per_repeat = (c2 - c1)/rate !pf_finish_time-pf_start_time
      ...
      ! print '("Rank "I0" done repeat "I0". ")',rank,i
    enddo
    elapse_time = elapse_time_per_repeat_sum/num_repeat_pf_run
    print '("Averaged time for pfasst run on rank "I0" = ",f7.3," seconds for "I0" repeats.")',rank,elapse_time,num_repeat_pf_run
    ...
    if(pf%rank==0) then
      print '("!>>>> Global average run time = ",f7.3,", max = ",f7.3,", min = ",f7.3," seconds.")',elapse_time_sum/nproc,elapse_time_max,elapse_time_min
    end if
