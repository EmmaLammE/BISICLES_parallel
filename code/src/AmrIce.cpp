...
AmrIce::initialize()
{
  ...
  // DFM (6/10/22) -- don't put the prefix on this one
  ...
  // DFM (6/10/22) -- put the prefix on this one
  string prefix = m_parmParsePrefix + "amr";
  ParmParse ppAmr(prefix);
  ...
  if (m_temporalAccuracy < 3)
    {
      ...
      // DFM (6/10/22) -- put the prefix on this one              
      string pph_prefix = m_parmParsePrefix + "amr.thickness_advection";
      ParmParse pph (pph_prefix);
...
/// compute the RHS (dh/dt) for an explicit update
/** note that this also does a velocity solve based on the current state
 */
void
AmrIce::explicitThicknessUpdate(Vector<LevelData<FArrayBox>* >& a_rhs,
                                Real a_time, Real a_dt)
{
  MayDay::Error("AmrIce::explicitThicknessUpdate not implemented");  
}

/// compute the RHS (dh/dt) for an implicit update
/** current not implemented */
void
AmrIce::implicitThicknessUpdate(Vector<LevelData<FArrayBox>* >& a_rhs,
                                Real a_time, Real a_dt)
{
  MayDay::Error("AmrIce::implicitThicknessUpdate not implemented");
}
...
// update  ice thickness *and* bedrock elevation
void
AmrIce::updateGeometryFromThickness(Vector<RefCountedPtr<LevelSigmaCS> >& a_vect_coordSys_new, 
                                    Vector<RefCountedPtr<LevelSigmaCS> >& a_vect_coordSys_old, 
                                    const Vector<LevelData<FArrayBox>* >& a_vectNewH, 
                                    Real a_dt)
{
  CH_TIME("AmrIce::updateGeometry");
  ...
  for (int lev=0; lev <= finestTimestepLevel() ; lev++)
    {
      DisjointBoxLayout& levelGrids = m_amrGrids[lev];
      LevelData<FArrayBox>& levelNewH = *a_vectNewH[lev];
      LevelSigmaCS& levelCoords = *(a_vect_coordSys_new[lev]);
      LevelData<FArrayBox>& levelNewHfromCS = levelCoords.getH();
      LevelData<FArrayBox>& levelOldHfromCS = (*a_vect_coordSys_old[lev]).getH();
      const RealVect& dx = levelCoords.dx();              
      ...
      for (dit.begin(); dit.ok(); ++dit)
        {
          const Box& gridBox = levelGrids[dit];
          FArrayBox& newH = levelNewH[dit];
          FArrayBox& oldHfromCS = levelOldHfromCS[dit];//(*m_old_thickness[lev])[dit];
          FArrayBox& newHfromCS = levelNewHfromCS[dit];//(*m_old_thickness[lev])[dit];          
	  ...
	  for (BoxIterator bit(gridBox); bit.ok(); ++bit)
	      ...
	      if (H < 0.0)
		{
          ...
          // copy new H int "official" LevelSigmaCS version
          newHfromCS.copy(newH);                              
  ...
  if (m_doInitialVelSolve) 
    {      
	  ...
	  if (m_initialGuessType == SlidingLaw)
	    ...
	    {
              // DFM (6/10/22) -- put the prefix on this one
              string ppString = m_parmParsePrefix + "amr";
	      ParmParse pp(ppString);
