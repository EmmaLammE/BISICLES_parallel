...
AmrIce::readCheckpointFile(HDF5Handle& a_handle)
{
  ...
  // DFM (6/10/22) -- put the prefix on this one
  string ppString2 = m_parmParsePrefix + "amr";
  ParmParse ppCheck(ppString2);
...
void AmrIce::writeMetaDataHDF5(HDF5Handle& a_handle) const
{
  ...
  // coordinate systsem data
  {
    ...
    ParmParse pp("CRS");
    int epsg = 0; // EPSG is the format I like, because it has a single integer value \todo WKT? PROJ.4? 
    pp.query("EPSG", epsg);
    if (epsg != 0)
      {
