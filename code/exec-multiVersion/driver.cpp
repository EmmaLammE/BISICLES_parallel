    ...
    for (int i=0; i<numCrseIntervals; i++)
      {
        ...
        cout << "coarse loop #  " << i << std::endl;
        Vector<LevelData<FluxBox>* > testtest(crseH.size(), NULL);
        for (int jjj=0;jjj<testtest.size();jjj++){
          const LevelData<FArrayBox>& testcurrentH = *crseH[jjj];
          //cout << "crseH jjj  " << jjj << " " << &testcurrentH << std::endl;
        }
      }
